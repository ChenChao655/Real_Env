from pymodbus.client.sync import ModbusTcpClient as ModbusClient  # pymodbus==2.5.3


class RGClient:

    def __init__(self, gripper_type, ip, port):
        ...
        if gripper_type not in ['rg2', 'rg6']:
            print("Please specify either rg2 or rg6.")
            return
        self.gripper_type = gripper_type  # RG2/6
        if self.gripper_type == 'rg2':
            self.max_width = 1100
            self.max_force = 400
        elif self.gripper_type == 'rg6':
            self.max_width = 1600
            self.max_force = 1200
    ...
    def set_target_width(self, width_val):
        ...
        """
        if self.gripper_type == "rg2":
            assert 0 <= width_val <= 1100
        elif self.gripper_type == "rg6":
            assert 0 <= width_val <= 1600
        else:
            assert False
    ...
    def move_gripper(self, width_val: int, force_val=400):
